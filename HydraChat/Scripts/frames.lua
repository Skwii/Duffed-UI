...
local InitNewFrame = function(self, event, msg, sender, guid) -- Create the new frame
	...
	Chatbox.title:SetFont(C.font, 14)
	Chatbox.title:SetShadowColor(0, 0, 0)
	Chatbox.title:SetShadowOffset(1.25, -1.25)
	Chatbox.title:SetPoint("BOTTOMLEFT", Chatbox, "TOPLEFT", 0, F.Scale(1))
	...
	Chatbox.edit:SetScript("OnMouseDown", function(self)
		...
		Chatbox.Editbox:SetBackdropBorderColor(chatColor.r, chatColor.g, chatColor.b)
	end)
	...
	F.CreatePanel(Chatbox.hideBG, 14, 14, "TOPRIGHT", Chatbox, "TOPRIGHT", -4, -4)
	...
	Chatbox.close:SetFont(C.font, 12)
	Chatbox.close:SetPoint("CENTER", 0, F.Scale(-1))
	Chatbox.close:SetText("X")
	
	Chatbox.miniBG = CreateFrame("Frame", nil, Chatbox)
	F.CreatePanel(Chatbox.miniBG, 14, 14, "RIGHT", Chatbox.hideBG, "LEFT", -3, 0)
	...
	Chatbox.mini:SetFont(C.font, 14)
	...
	for key, frame in ipairs(F.chats) do
		if not frame.minimized then
			local point, relativeTo, relativePoint, xOfs, yOfs = F.chats[key]:GetPoint() -- Use the point rather than anchoring to the frame
